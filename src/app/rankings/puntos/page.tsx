...
import { getRankingPuntos, getTemporadasDisponibles, type FilaRankingPuntos } from "@/lib/queries";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const title = "Ranking de puntos · Club Vicentinos";
  const description = lider
    ? `${filas.length} jugadores. Líder histórico: ${lider.nombre}, con ${lider.puntos} puntos.`
    : `Ranking histórico de puntos, jugador por jugador.`;
...
function ordenarFilas(filas: FilaRankingPuntos[], orden: OrdenKey): FilaRankingPuntos[] {
  return [...filas].sort(
    (a, b) => b[orden] - a[orden] || b.puntos - a.puntos || a.nombre.localeCompare(b.nombre)
  );
}

export default async function RankingPuntosPage({
  searchParams,
}: {
  ...
  const orden: OrdenKey = esOrdenKey(ordenParam) ? ordenParam : "puntos";

  const [temporadas, filasSinOrdenar] = await Promise.all([
    getTemporadasDisponibles(),
    getRankingPuntos(temporada),
  ]);
  ...
  function hrefOrden(key: OrdenKey) {
    ...
    if (key !== "puntos") params.set("orden", key);
    const qs = params.toString();
    return qs ? `/rankings/puntos?${qs}` : "/rankings/puntos";
  }

  return (
    <main className="mx-auto max-w-3xl p-6">
      <h1 className="text-2xl font-semibold text-navy-dark">Ranking de puntos</h1>
      <p className="mt-1 text-sm text-navy/70">
        {temporada ? `Temporada ${temporada}` : "Histórico (todas las temporadas)"}
      </p>

      <div className="mt-4">
        <TemporadaTabs basePath="/rankings/puntos" temporadas={temporadas} temporadaActiva={temporada} />
      </div>
          ...
          <thead>
            <tr className="bg-navy text-left text-white">
              ...
              {ORDEN_OPCIONES.map((o) => (
                <th key={o.key} className={`py-2 pr-2 text-right ${o.key === "puntos" ? "font-semibold" : ""}`}>
                  <Link
          ...
          <tbody>
            {filas.map((f, i) => (
              <tr key={f.jugadorId} className="border-b border-navy-light/60 odd:bg-navy-light">
                ...
                <td className={`py-1.5 pr-2 text-right text-navy-dark ${orden === "tries" ? "font-semibold" : ""}`}>{f.tries}</td>
                <td className={`py-1.5 pr-2 text-right text-navy-dark ${orden === "conversiones" ? "font-semibold" : ""}`}>{f.conversiones}</td>
                <td className={`py-1.5 pr-2 text-right text-navy-dark ${orden === "penales" ? "font-semibold" : ""}`}>{f.penales}</td>
                <td className={`py-1.5 pr-2 text-right text-navy-dark ${orden === "drops" ? "font-semibold" : ""}`}>{f.drops}</td>
