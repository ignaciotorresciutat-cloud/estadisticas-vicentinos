...
async function getRankingsHistoricos(): Promise<RankingsHistoricos> {
  const [filasPuntos, capsRows] = await Promise.all([
    getRankingPuntos(),
...
export async function getListaJugadores(): Promise<FilaListaJugador[]> {
  const jugadores = await prisma.jugador.findMany({
  ...
  return jugadores.map((j) => ({
    ...
    totalCaps: j._count.participaciones,
  }));
}
...
export async function getResumenClub(): Promise<ResumenClub> {
  const [temporadas, partidos, triesAgg, jugadores, amarillas, rojas] = await Promise.all([
...
export async function getHistorialGeneral(): Promise<FilaHistorialGeneral[]> {
  ...
  for (const p of partidos) {
    ...
    const bucket = p.condicion === "LOCAL" ? fila.local : fila.visitante;
    for (const w of [fila.total, bucket]) {
      w.j++;
      if (p.resultadoPropio > p.resultadoRival) w.g++;
      else if (p.resultadoPropio < p.resultadoRival) w.p++;
      else w.e++;
    }
...
export type DetalleRival = {
  clubId: number;
  club: string;
  total: WLD;
  local: WLD;
  visitante: WLD;
  partidos: PartidoVsRival[];
  masPresencias: { jugadorId: number; nombre: string; cantidad: number } | null;
  masTries: { jugadorId: number; nombre: string; cantidad: number } | null;
  masVictorias: { jugadorId: number; nombre: string; cantidad: number } | null;
};

export async function getDetalleRival(clubId: number): Promise<DetalleRival | null> {
  const club = await prisma.club.findUnique({ where: { id: clubId } });
  if (!club) return null;

  const partidos = await prisma.partido.findMany({
    where: { rivalId: clubId },
    select: {
      id: true,
      fecha: true,
      condicion: true,
  ...
  const total = wldVacio();
  const local = wldVacio();
  const visitante = wldVacio();
  const partidoGanado = new Map<number, boolean>();
  for (const p of partidos) {
    const bucket = p.condicion === "LOCAL" ? local : visitante;
    const gano = p.resultadoPropio > p.resultadoRival;
    partidoGanado.set(p.id, gano);
    for (const w of [total, bucket]) {
      w.j++;
      if (gano) w.g++;
      else if (p.resultadoPropio < p.resultadoRival) w.p++;
      else w.e++;
  ...
  const [participaciones, tries] = await Promise.all([
    prisma.participacion.findMany({
      where: { partidoId: { in: partidoIds }, jugador: { esJugadorReal: true } },
      select: { partidoId: true, jugadorId: true, jugador: { select: { nombreCanonico: true } } },
    }),
    prisma.puntuacion.findMany({
      where: { partidoId: { in: partidoIds }, tipo: "TRY", jugador: { esJugadorReal: true } },
      select: { cantidad: true, jugadorId: true, jugador: { select: { nombreCanonico: true } } },
    }),
  ]);

  const presenciasPorJugador = new Map<number, { jugadorId: number; nombre: string; cantidad: number }>();
  const victoriasPorJugador = new Map<number, { jugadorId: number; nombre: string; cantidad: number }>();
  for (const part of participaciones) {
    const p1 = presenciasPorJugador.get(part.jugadorId) ?? {
      jugadorId: part.jugadorId,
      nombre: part.jugador.nombreCanonico,
      cantidad: 0,
    };
    p1.cantidad++;
    presenciasPorJugador.set(part.jugadorId, p1);

    if (partidoGanado.get(part.partidoId)) {
      const p2 = victoriasPorJugador.get(part.jugadorId) ?? {
        jugadorId: part.jugadorId,
        nombre: part.jugador.nombreCanonico,
        cantidad: 0,
      };
      p2.cantidad++;
      victoriasPorJugador.set(part.jugadorId, p2);
    }
  }

  const triesPorJugador = new Map<number, { jugadorId: number; nombre: string; cantidad: number }>();
  for (const t of tries) {
    const p = triesPorJugador.get(t.jugadorId) ?? {
      jugadorId: t.jugadorId,
      nombre: t.jugador.nombreCanonico,
      cantidad: 0,
    };
    p.cantidad += t.cantidad;
    triesPorJugador.set(t.jugadorId, p);
  }

  function maximo(map: Map<number, { jugadorId: number; nombre: string; cantidad: number }>) {
    let max: { jugadorId: number; nombre: string; cantidad: number } | null = null;
    for (const v of map.values()) {
      if (!max || v.cantidad > max.cantidad) max = v;
    }
    return max;
  }
  ...
  return {
    ...
    masPresencias: maximo(presenciasPorJugador),
    masTries: maximo(triesPorJugador),
    masVictorias: maximo(victoriasPorJugador),
  };
}
...
export type Records = {
  ...
  temporadaMasTries: RecordTemporada | null;
...
export async function getRecords(): Promise<Records> {
  ...
  // temporada con más tries anotados
  const triesPorTemporadaMap = new Map<number, number>();
  for (const p of partidos) {
    const t = triesPorPartido.get(p.id) ?? 0;
    triesPorTemporadaMap.set(p.temporada, (triesPorTemporadaMap.get(p.temporada) ?? 0) + t);
  }
  let temporadaMasTries: RecordTemporada | null = null;
  for (const [temporada, cantidad] of triesPorTemporadaMap) {
    if (!temporadaMasTries || cantidad > temporadaMasTries.cantidad) {
      temporadaMasTries = { temporada, cantidad };
    }
  }
...
export async function getCamadasResumen(): Promise<FilaCamada[]> {
  const jugadores = await prisma.jugador.findMany({
    where: { esJugadorReal: true, camada: { not: null } },
    select: {
      camada: true,
      _count: { select: { participaciones: true, tarjetas: true } },
      puntos: { select: { tipo: true, cantidad: true } },
    },
  });

  const porCamada = new Map<number, FilaCamada>();
  for (const j of jugadores) {
    const camada = j.camada!;
    const fila = porCamada.get(camada) ?? {
      camada,
      jugadoresConCaps: 0,
      presencias: 0,
      tries: 0,
    ...
    if (j._count.participaciones > 0) fila.jugadoresConCaps++;
    fila.presencias += j._count.participaciones;
    fila.tarjetas += j._count.tarjetas;
    for (const pt of j.puntos) {
      if (pt.tipo === "TRY") fila.tries += pt.cantidad;
      fila.puntos += (PUNTOS_POR_TIPO[pt.tipo] ?? 0) * pt.cantidad;
    }
    porCamada.set(camada, fila);
  }

  return [...porCamada.values()].sort((a, b) => a.camada - b.camada);
}

export type JugadorCamada = {
  id: number;
  nombre: string;
  capsTitular: number;
  capsSuplente: number;
...
export async function getDetalleCamada(camada: number): Promise<{ camada: number; jugadores: JugadorCamada[] } | null> {
  const jugadores = await prisma.jugador.findMany({
    where: { esJugadorReal: true, camada },
    select: {
      id: true,
      nombreCanonico: true,
      participaciones: { select: { rol: true, partido: { select: { fecha: true } } } },
      puntos: { select: { tipo: true, cantidad: true } },
  ...
  const filas: JugadorCamada[] = jugadores.map((j) => {
    const capsTitular = j.participaciones.filter((p) => p.rol === "TITULAR").length;
    const capsSuplente = j.participaciones.length - capsTitular;
    const fechaDebut = j.participaciones
      .filter((p) => p.rol === "TITULAR")
      .reduce<Date | null>((min, p) => (!min || p.partido.fecha < min ? p.partido.fecha : min), null);
    let tries = 0;
    let puntos = 0;
    for (const pt of j.puntos) {
      if (pt.tipo === "TRY") tries += pt.cantidad;
      puntos += (PUNTOS_POR_TIPO[pt.tipo] ?? 0) * pt.cantidad;
    }
    return {
      id: j.id,
      nombre: j.nombreCanonico,
      capsTitular,
      capsSuplente,
      capsTotal: capsTitular + capsSuplente,
      tries,
      puntos,
      tarjetas: j._count.tarjetas,
      fechaDebut,
    };
  });

  filas.sort((a, b) => b.capsTotal - a.capsTotal || a.nombre.localeCompare(b.nombre));
  return { camada, jugadores: filas };
}
