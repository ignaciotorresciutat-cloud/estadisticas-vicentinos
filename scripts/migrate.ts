/**
 * Migración del Excel crudo (partido a partido) + tablas de equivalencia -> SQLite.
 *
 * Solo LEE el excel original, nunca lo modifica.
...
async function main() {
  ...
  console.log(`Partidos parseados: ${allPartidos.length}. Sin nombres sin mapear. Cargando la base...`);

  // -------- carga a la base (reset + reinsert) --------
  ...
  async function getClubId(nombreCanonico: string): Promise<number> {
    if (clubIds.has(nombreCanonico)) return clubIds.get(nombreCanonico)!;
  ...
  async function getJugadorId(nombreCanonico: string): Promise<number> {
    if (jugadorIds.has(nombreCanonico)) return jugadorIds.get(nombreCanonico)!;
  ...
  console.log("\n✅ Migración completa. Números de control:");
